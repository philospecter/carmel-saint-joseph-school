...
CREATE POLICY "year_read_all" ON public.academic_years FOR SELECT TO authenticated USING (true);
CREATE POLICY "year_admin_all" ON public.academic_years FOR ALL TO authenticated
...
CREATE TRIGGER academic_years_touch BEFORE UPDATE ON public.academic_years FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();
...
ALTER TABLE public.student_enrollments ADD COLUMN is_graduated boolean NOT NULL DEFAULT false;

DO $$
DECLARE t text; seed uuid;
BEGIN
  SELECT id INTO seed FROM public.academic_years WHERE is_current;
  FOREACH t IN ARRAY ARRAY['student_enrollments','grades','attendance','teacher_assignments','homework','homework_submissions','announcements'] LOOP
...
CREATE POLICY "year_restrict" ON public.student_enrollments AS RESTRICTIVE FOR ALL TO authenticated
...
CREATE POLICY "year_restrict" ON public.grades AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id())
  WITH CHECK (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id());
CREATE POLICY "year_restrict" ON public.attendance AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id())
  WITH CHECK (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id());
CREATE POLICY "year_restrict" ON public.teacher_assignments AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id())
  WITH CHECK (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id());
CREATE POLICY "year_restrict" ON public.homework AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id())
  WITH CHECK (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id());
CREATE POLICY "year_restrict" ON public.homework_submissions AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id())
  WITH CHECK (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id());
CREATE POLICY "year_restrict" ON public.announcements AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id())
  WITH CHECK (public.has_role(auth.uid(),'admin') OR academic_year_id = public.current_academic_year_id());

CREATE OR REPLACE FUNCTION public.start_new_academic_year(_label text)
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE old_id uuid; new_id uuid; caller uuid := auth.uid();
BEGIN
  ...
  INSERT INTO public.academic_years(label, is_current) VALUES (_label, true) RETURNING id INTO new_id;
  ...
  FOR r IN SELECT user_id, stage_group, grade_level FROM public.student_enrollments WHERE academic_year_id = old_id AND is_graduated = false LOOP
    ...
    IF r.user_id = ANY(_repeats) THEN
      INSERT INTO public.student_enrollments(user_id, stage_group, grade_level, academic_year_id, is_graduated)
        VALUES (r.user_id, r.stage_group, r.grade_level, new_id, false); CONTINUE;
    END IF;
    IF r.grade_level = 'sec3' THEN
      INSERT INTO public.student_enrollments(user_id, stage_group, grade_level, academic_year_id, is_graduated)
        VALUES (r.user_id, r.stage_group, r.grade_level, new_id, true); CONTINUE;
    END IF;
