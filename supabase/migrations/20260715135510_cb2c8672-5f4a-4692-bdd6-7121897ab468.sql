...
-- 1) Rewrite promote_students: graduate sec3 in the OLD year, do not enroll in new year
...
BEGIN
  ...
  FOR r IN SELECT user_id, stage_group, grade_level FROM public.student_enrollments WHERE academic_year_id=old_id AND is_graduated=false LOOP
    ...
    IF r.grade_level='sec3' THEN
      UPDATE public.student_enrollments SET is_graduated = true
        WHERE user_id = r.user_id AND academic_year_id = old_id;
      CONTINUE;
    END IF;
...
-- 2) Rewrite start_year_and_promote with the same fix
CREATE OR REPLACE FUNCTION public.start_year_and_promote(_label text, _promotions jsonb, _repeats uuid[])
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- 3) Backfill existing bad data:
--    Move the "graduated" flag from wrongly-created new-year sec3 rows onto the matching old-year rows.
WITH cur AS (SELECT id FROM public.academic_years WHERE is_current LIMIT 1),
     prev AS (SELECT id FROM public.academic_years WHERE closed_at IS NOT NULL ORDER BY closed_at DESC LIMIT 1),
     bad AS (
       SELECT se.user_id FROM public.student_enrollments se, cur
       WHERE se.academic_year_id = cur.id AND se.grade_level = 'sec3' AND se.is_graduated = true
     )
UPDATE public.student_enrollments se
   SET is_graduated = true
  FROM bad, prev
 WHERE se.user_id = bad.user_id
   AND se.academic_year_id = prev.id
   AND se.grade_level = 'sec3';

DELETE FROM public.student_enrollments se
 USING (SELECT id FROM public.academic_years WHERE is_current LIMIT 1) cur
 WHERE se.academic_year_id = cur.id
   AND se.grade_level = 'sec3'
   AND se.is_graduated = true;
