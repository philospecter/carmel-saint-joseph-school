...
FROM (VALUES
  ('primary_1_2','p1'), ('primary_1_2','p2'),
  ('primary_3_6','p3'), ('primary_3_6','p4'), ('primary_3_6','p5'), ('primary_3_6','p6')
) AS s(stage_group, grade_level),
...
CREATE POLICY "Student can read own grades" ON public.grades
  FOR SELECT TO authenticated
  USING (auth.uid() = student_id);

CREATE POLICY "Admin full grades" ON public.grades
  FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Stage manager manages grades in stage" ON public.grades
  FOR ALL TO authenticated
  USING (public.student_of_stage_manager(student_id, auth.uid()))
  WITH CHECK (public.student_of_stage_manager(student_id, auth.uid()));
...
CREATE TRIGGER grades_updated_at BEFORE UPDATE ON public.grades
  FOR EACH ROW EXECUTE FUNCTION public.grades_touch_updated_at();
