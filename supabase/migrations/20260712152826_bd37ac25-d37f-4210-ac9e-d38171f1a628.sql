...
-- 1) Term enum: add midyear + final
ALTER TYPE public.term_type ADD VALUE IF NOT EXISTS 'midyear';
ALTER TYPE public.term_type ADD VALUE IF NOT EXISTS 'final';

-- 2) Subjects: add max_score (nullable, no default; admins must set explicitly)
ALTER TABLE public.subjects
  ADD COLUMN IF NOT EXISTS max_score numeric NULL
  CHECK (max_score IS NULL OR max_score > 0);

-- 3) Grades: allow NULL month for midyear/final
...
ALTER TABLE public.grades
  ADD CONSTRAINT grades_month_range_chk
  CHECK (month IS NULL OR (month BETWEEN 1 AND 12));

-- Replace the unique constraint with partial unique indexes
ALTER TABLE public.grades DROP CONSTRAINT IF EXISTS grades_student_id_subject_id_term_month_key;
CREATE UNIQUE INDEX IF NOT EXISTS grades_uniq_monthly
  ON public.grades (student_id, subject_id, term, month)
  WHERE month IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS grades_uniq_termly
  ON public.grades (student_id, subject_id, term)
  WHERE month IS NULL;

-- 4) Trigger: enforce month/term consistency + max_score
...
BEGIN
  ...
  IF s_max IS NULL THEN
    RAISE EXCEPTION 'Subject has no max_score set; admin must configure it before grading';
  END IF;
  IF NEW.score < 0 OR NEW.score > s_max THEN
    RAISE EXCEPTION 'Score % is outside allowed range 0..%', NEW.score, s_max;
  END IF;
...
CREATE TRIGGER grades_validate_biu
  BEFORE INSERT OR UPDATE ON public.grades
  FOR EACH ROW EXECUTE FUNCTION public.grades_validate();

-- 5) Term month settings table
...
CREATE POLICY "tms_read" ON public.term_month_settings
  FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "tms_admin_write" ON public.term_month_settings;
CREATE POLICY "tms_admin_write" ON public.term_month_settings
